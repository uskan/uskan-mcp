...
const server = new McpServer(
  { name: "uskan-mcp", version: "0.6.0" },
  {
    ...
    instructions: [
      ...
      "",
      "DECIDE, DO NOT ASK. You are sitting in the user's repository with a",
      "terminal. Almost every question you are tempted to put to them, you can",
      "answer yourself by looking, and every question you ask is effort we",
      "promised to remove. In particular:",
      "",
      "  How to produce the build — LOOK, then say what you are doing:",
      "    eas.json present            → `eas build -p android --profile production`",
      "    android/ directory present  → `./gradlew bundleRelease` in android/",
      "    Expo, no android/           → `npx expo prebuild -p android` first",
      "    pubspec.yaml                → `flutter build appbundle --release`",
      "    an .aab/.ipa already on disk → just use it",
      "  Only involve the user when signing needs THEIR secret: an Android",
      "  keystore they have not created, or Apple certificates.",
      "",
      "  Which Play track — do not ask. Use `internal` and TELL them you did.",
      "  Internal testing is invisible to the public and skips review, so it is",
      "  the safe first upload for every app. Ask only before `production`,",
      "  because that one is public and hard to walk back.",
      "",
...
server.registerTool(
  "uskan_upload_build",
  {
    ...
    description: [
      ...
      "",
      "uskan does not build the app in this mode, but YOU can. Run the build",
      "yourself rather than asking the user which method to use — the repository",
      "tells you. Where the output lands:",
      "  eas build      → downloads a URL, or `eas build:download`",
      "  gradle         → android/app/build/outputs/bundle/release/app-release.aab",
      "  flutter        → build/app/outputs/bundle/release/app-release.aab",
      "  Xcode archive  → wherever the user exported the .ipa",
      "Announce the command before running it; do not make the user choose it.",
    ].join("\n"),
    inputSchema: {
      projectId: z
        .string()
...
server.registerTool(
  "uskan_publish",
  {
    ...
    description: [
      ...
      "",
      "Track choice: do NOT ask for the first upload. Use 'internal' for Android",
      "and 'testflight' for iOS, and say that is what you did. Both are private,",
      "neither goes to review, and both can be promoted afterwards — so there is",
      "nothing for the user to weigh up.",
      "",
      "'production' and 'appstore' ARE different: they are public and awkward to",
      "undo. Send a release there only when the user asks for it in so many words.",
    ].join("\n"),
    inputSchema: {
      projectId: z.string().trim().min(1).describe("uskan project id."),
